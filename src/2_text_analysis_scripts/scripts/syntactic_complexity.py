...
def main():
    ...
    for i, row in df.iterrows():
        ...
        h = compute_metrics(nlp, row.get(cfg["human_col"]), cfg["min_tokens"])
        l = compute_metrics(nlp, row.get(cfg["llm_col"]), cfg["min_tokens"])
    ...
    out_path = os.path.join(OUT_DIR, f"syntactic_complexity_{args.dataset}.csv")
