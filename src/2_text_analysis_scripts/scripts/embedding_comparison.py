...
def main():
    ...
    df_all = read_csv_robust(cfg["csv"])
    ...
    for cond, llm_col in resolve_conditions(args.dataset, args.condition):
        ...
        df = df_all[df_all[cfg["human_col"]].notna() & df_all[llm_col].notna()].reset_index(drop=True)
        human = df[cfg["human_col"]].astype(str).tolist()
        llm = df[llm_col].astype(str).tolist()
        n = len(df)
        print(f"\n[{cond or 'generated'}] {args.dataset}: {n} paired documents")
        ...
        ]).to_csv(os.path.join(CSV_DIR, f"embedding_twosample_{tag}.csv"), index=False, encoding="utf-8")
