...
All formal feature scripts accept `--dataset {formal,informal}` and
`--condition {baseline,human_like,detector_aware,all}` (default `all`; ignored for informal).
Output naming: baseline/informal keep the legacy `_{dataset}` name; the adversarial conditions
get `_{dataset}_{condition}` files (spaCy writes per-condition subdirectories under `results/`).

| Script | Output | Features |
|---|---|---|
| `spacy_linguistic_analysis.py` | `results/{dataset}[/{condition}]/linguistic_analysis_*.csv` | POS, dependency & NER distributions, tokens, POS proportions/differences |
...
| `distribution_distances.py` | `csv_files/distribution_distances_*.csv` | Jensen–Shannon divergence, Wasserstein distance across feature distributions |
| `multi_layer_analysis.py` | `analysis_runs/` | Morphology, sentence-transformer embeddings + PCA, topic modelling, readability |
| `significance_tests.py` | `csv_files/significance_tests_results.csv` | Hypothesis tests across features (apply multiple-comparison correction) |
...
Run any feature script from the repo root (defaults to all three formal conditions):
```
...
python src/2_text_analysis_scripts/scripts/distribution_distances.py --dataset both
```

Restrict to one condition with e.g. `--condition detector_aware`. Use `--dataset informal`
for the comment register.
